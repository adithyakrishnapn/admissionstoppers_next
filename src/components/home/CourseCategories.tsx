        ...
        <div className="grid lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 grid grid-cols-12 gap-6">
            {categories.map((cat, i) => (
              <Reveal key={cat.title} delay={i * 0.06} y={24} className={cat.className}>
                <Link
                  href={cat.href}
                  className="group relative block h-full overflow-hidden rounded-2xl border border-gray-100/50 shadow-sm"
                >
                  <Image
                    src={cat.image}
                    alt={cat.title}
          ...
          <div className="lg:col-span-5 flex flex-col h-full">
            {sideCategories.map((cat, i) => (
              <Reveal key={cat.title} delay={0.12 + i * 0.08} y={24} className={`w-full ${cat.className}`}>
                <Link
                  href={cat.href}
                  className="group relative block h-full w-full overflow-hidden rounded-2xl border border-gray-100/50 shadow-sm"
                >
                  <Image
                    src={cat.image}
                    alt={cat.title}
